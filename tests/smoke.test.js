...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Photographs")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/bfff2eb8-84d2-5740-8e2c-17b970a1f6dc')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/aabb4c5f-7b18-5d94-a2c9-d82eb2eb1616')
    ...
    const isl = await mountSite('#/item/bfff2eb8-84d2-5740-8e2c-17b970a1f6dc')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === 'bfff2eb8-84d2-5740-8e2c-17b970a1f6dc')
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    const item = items.find((i) => i.project_id === islProjectId) ?? { internal_name: "Photograph." }
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/0758a11b-67ab-5ddc-a953-275a1d7a45b8')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain("Serbia")
    expect(host.textContent).toContain('Museum of Applied Art')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/f4b9cc0b-85a9-57a3-8bbc-d3741afcb48f')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Museum of Applied Art'), { timeout: 20000 })
    await vi.waitFor(() => expect(host.textContent).toContain("Belgrade"), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/f4b9cc0b-85a9-57a3-8bbc-d3741afcb48f/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['photographs', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
