...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('photographs.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('photographs.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('photographs.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
